...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    // intercepets http request and gets jwt token
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
            throws ServletException, IOException {

        String token = getTokenFromRequest(request);
        if (StringUtils.hasText(token) && _jwtTokenProvider.validateToken(token)) {
            String username = _jwtTokenProvider.getUsername(token);
            UserDetails userDetails = _appUserDetailsService.loadUserByUsername(username);
            UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                    userDetails, null, userDetails.getAuthorities());
            authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));

            // sends token to spring security context
